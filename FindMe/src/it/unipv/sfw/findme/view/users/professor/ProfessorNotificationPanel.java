...
public class ProfessorNotificationPanel extends JPanel{
	...
	public ProfessorNotificationPanel(Users user, UserGUI frame) {
		...
		SwapNotificationDAO swapDao=new SwapNotificationDAO();
			...
			List<Notification> listSwap=swapDao.getNotifications(new SwapNotification(user.getID(), user.getID(), null, null, null, null, null));
		...
		for(int i=0; i<user.getNotifications().size(); i++) {
			try {
				...
				SwapNotification draft=(SwapNotification)user.getNotifications().get(i);
		...
		JList<SwapNotification> listDraft=new JList(swapNotAcceptedNotifications.toArray());
		...
		acceptDraft.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					SwapNotification swap=listDraft.getSelectedValue();
		...
		JList<SwapNotification> listReminders=new JList(swapAcceptedNotifications.toArray());
